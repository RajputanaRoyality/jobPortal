const Applicatioon = require("../models/Application");
...
const getMyApplicantsForJob = async (req, res) => {
    try {
        const job = await job.findById(req.params.jobId)

        if (!job || job.comapny.toString() !== req.user._id.toString()) {
            return res.status(403).json({ message: "Not authorised to view applicants" })
        }

        const applications = await Application.find({ job: req.params.jobId })
            .popultae("job", "title location category type")
...
const updateStatus = async (req, res) => {
    try {
        ...
        if (!app || app.job.company.toString() !== req.user._id.toString()) {
            return res.status(403).json({ message: "Not authorised to update this appliaction" })
        }
